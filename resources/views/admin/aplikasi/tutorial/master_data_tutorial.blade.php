...
<div class="text-sm font-medium text-center text-gray-500 border-b border-gray-200 dark:text-gray-400 dark:border-gray-700">
    <ul class="flex flex-wrap -mb-px">
        ...
        <li class="me-2">
            <a href="{{route('admin.ttm.data_lokasi')}}" class="inline-block p-4 border-b-2 border-transparent rounded-t-lg hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300">Data Lokasi</a>
        </li>
        <li class="me-2">
            <a href="{{route('admin.ttm.data_tutorial')}}" class="inline-block p-4 text-blue-600 border-b-2 border-blue-600 rounded-t-lg active dark:text-blue-500 dark:border-blue-500" aria-current="page">Data Tutorial</a>
        </li>
        <li class="me-2">
        ...
        <table class="table w-full text-sm text-left text-gray-500 dark:text-gray-400">
            <thead class="sticky top-0 text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                    ...
                    <th scope="col" class="px-4 py-3 uppercase">kode tutorial</th>
                    <th scope="col" class="px-4 py-3 uppercase">kode prodi</th>
                    <th scope="col" class="px-4 py-3 uppercase">nama prodi</th>
                    <th scope="col" class="px-4 py-3 uppercase">kode matakuliah</th>
                    <th scope="col" class="px-4 py-3 uppercase">nama matakuliah</th>
                    <th scope="col" class="px-4 py-3 uppercase">kode lokasi</th>
                    <th scope="col" class="px-4 py-3 uppercase">nama lokasi</th>
                    <th scope="col" class="px-4 py-3 uppercase">kode kabko</th>
                    <th scope="col" class="px-4 py-3 uppercase">nama kabko</th>
                </tr>
            </thead>
            <tbody>
                @if(isset($data))
                ...
                <tr class="border-b dark:border-gray-700">
                    ...
                    <td class="px-4 py-3">{{$row->kode_tutorial}}</td>
                    <td class="px-4 py-3">{{$row->kode_prodi}}</td>
                    <td class="px-4 py-3">{{$row->nama_prodi}}</td>
                    <td class="px-4 py-3">{{$row->kode_matakuliah}}</td>
                    <td class="px-4 py-3">{{$row->nama_matakuliah}}</td>
                    <td class="px-4 py-3">{{$row->kode_lokasi}}</td>
                    <td class="px-4 py-3">{{$row->nama_lokasi}}</td>
                    <td class="px-4 py-3">{{$row->kode_kabko}}</td>
                    <td class="px-4 py-3">{{$row->nama_kabko}}</td>
                </tr>
                @endforeach
                @endif
            </tbody>
...
<div id="modal-import" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-md max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <!-- Modal body -->
            <form class="p-4 md:p-5" action="{{route('admin.ttm.data_tutorial_import')}}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="grid gap-4 mb-4 grid-cols-2">
                    <div class="col-span-2">
                        <label for="description" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Import File Excel Data Tutorial</label>
