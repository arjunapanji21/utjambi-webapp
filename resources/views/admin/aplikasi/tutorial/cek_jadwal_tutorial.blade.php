        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse list-none">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <a href="#"
                        class="ms-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ms-2 dark:text-gray-400 dark:hover:text-white">{{$active}}</a>
                </div>
            </li>
...
<div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
    <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
        <div class="w-full md:w-1/2">
            <form class="flex items-center" method="GET" action="{{ route('admin.ttm.cek_jadwal_tutorial') }}">
                <div class="relative w-full">
                    ...
                    <input type="text" name="search" id="search" value="{{ request('search') }}"
                        ...
                        placeholder="Cari berdasarkan NIM, nama, kelas...">
                </div>
                <button type="submit" class="ml-2 text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
                    Search
                </button>
                @if(request('search'))
                <a href="{{ route('admin.ttm.cek_jadwal_tutorial') }}" class="ml-2 text-gray-700 bg-white border border-gray-300 hover:bg-gray-50 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-gray-600 dark:hover:bg-gray-700 dark:text-white focus:outline-none dark:focus:ring-primary-800">
                    Clear
                </a>
                @endif
            </form>
    ...
    <div class="overflow-auto px-4">
        ...
        <table class="table w-full text-sm text-left text-gray-500 dark:text-gray-400">
            <thead class="sticky top-0 text-xs text-gray-700 uppercase bg-gray-100 dark:bg-gray-700 dark:text-gray-400 shadow-sm">
                <tr>
                    <th scope="col" class="px-4 py-4 font-semibold">#</th>
                    <th scope="col" class="px-4 py-4 font-semibold">Masa</th>
                    <th scope="col" class="px-4 py-4 font-semibold">Nim</th>
                    <th scope="col" class="px-4 py-4 font-semibold">Nama</th>
                    <th scope="col" class="px-4 py-4 font-semibold">Kelas</th>
                    <th scope="col" class="px-4 py-4 font-semibold">Matakuliah</th>
                    <th scope="col" class="px-4 py-4 font-semibold">Tutor</th>
                    <th scope="col" class="px-4 py-4 font-semibold">Lokasi</th>
                    <th scope="col" class="px-4 py-4 font-semibold">Jadwal</th>
                </tr>
            </thead>
            <tbody>
                @if(isset($data))
                ...
                <tr class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    ...
                    <td class="px-4 py-3">{{$row->nama_matakuliah}}</td>
                    <td class="px-4 py-3">{{$row->nama_tutor}}</td>
                    <td class="px-4 py-3">{{$row->lokasi}}</td>
                    <td class="px-4 py-3">{{$row->jadwal}}</td>
                </tr>
                @endforeach
                @endif
            </tbody>
...
<div id="modal-import" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-md max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <!-- Modal body -->
            <form class="p-4 md:p-5" action="{{route('admin.ttm.cek_jadwal_tutorial_import')}}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="grid gap-4 mb-4 grid-cols-2">
                    <div class="col-span-2">
                        <label for="description" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Import File Excel Dari Laman tutorial.ut.ac.id</label>
...
<script type="module">
    ...
    function performSearch() {
        ...
        var entries = parseInt($("#entries").val());
        var visibleCount = 0;
        ...
        $("table tr").each(function (index) {
            ...
            if (rowVisible && visibleCount < entries) {
                $(this).show();
                visibleCount++;
            } else {
                $(this).hide();
